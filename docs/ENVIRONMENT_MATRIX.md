...
| Kuru | Monad Testnet MON-USDC market `0xa241896A7Dbe8a550D2E5fF7A914bB1989ceD2D9`; USDC `0x3bA3d39AFcf8bb994f7964B3e0171Ea2Ba361570` | Contracts `2060bb2736080c175d80d568bfdb6226bb5abd04`; SDK/ABI `636509c2eafd63479d3f399703354e0d09f51e18`; deployed implementation remains source-unverified | Public Foundation RPC used; funded test wallet required only for later authorized write | ABI-compatible `getMarketParams()` and `getL2Book()` calls; explorer/source probe | BLOCKED (narrow read ABI verified) | Native MON base, USDC quote, scales/bounds/current fees and empty manual L2 snapshot recorded in `docs/KURU_COMPATIBILITY_INVESTIGATION.md`; exact build/settlement behavior remains blocked |
