...
| CRE | Workflow plus Kairos receiver; Monad Testnet/forwarder `UNVERIFIED` | CLI updated locally to `v1.34.0`; npm `@chainlink/cre-sdk` `1.21.1` | Local authenticated account confirmed; workflow configuration/key material not stored | `cre update`, `cre version`, `cre whoami`, `cre workflow --help` | BLOCKED | Verify Monad forwarder/network, initialize a minimal workflow, and run a no-deployment simulation |
