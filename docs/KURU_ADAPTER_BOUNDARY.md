...
The 2026-09-18 follow-up expanded the fixed-fork suite to six cases with a successful FOK full fill in addition to the existing non-FOK partial fill, FOK failure, minimum-output failure, actual-minimum-fill rollback, and effective-price rollback. This closes the behavioral case list in the mentor request on a controlled fork; it does not supply build provenance or public authorization.
