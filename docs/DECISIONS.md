...
- 2026-09-16 — M1 is complete under the written `WORKPLAN.md` exit: local policy correctness and Kuru adapter behavior against a documented fixed fork are both evidenced. Exact deployed-source/build equivalence is not an M1 exit criterion; it remains a separate prerequisite for reconsidering the public execution interlock. No public deployment or transaction is implied.
...
- 2026-09-16 — M2 freshness is fail-closed: maximum snapshot age `10s`, proposal validity `5s`, per-request timeout `2000ms`, and at most three attempts with `250ms`/`500ms` backoff. These bounds follow a 24-block Monad Testnet sample (p95/max timestamp interval `1s`) plus observed individual RPC success; successful retries are still rechecked for freshness.
- 2026-09-16 — M2 is complete under the written `WORKPLAN.md` exit. Auditable proposals and WAIT decisions cover M2.1–M2.6 and ENG-01–03, while the M1 contract remains final authority over lifecycle, nonce, budget, actual settlement, minimum fill, and effective price. Manual L2 is the only included capacity source; AMM vault liquidity remains excluded and may make the estimator conservatively understate capacity.
- 2026-09-16 — The CRE receiver uses the official `IReceiver` surface and packed metadata layout pinned from `smartcontractkit/cre-templates` revision `d0223f3`. Kairos narrows the generic template: trusted forwarder and activation authority are immutable, policy/workflow identity activates exactly once with no weakening setters, duplicate report hashes are rejected, and every valid report still enters `KairosPolicy.execute` for final nonce, lifecycle, budget, and settlement enforcement.
- 2026-09-16 — The CRE workflow imports the M2 parser and decision engine rather than reproducing policy math. RPC reads are pinned to one block and use identical consensus over the serialized acquisition. Runtime evaluation uses the later of DON consensus time and the pinned block timestamp to tolerate bounded clock lead; the unchanged 10-second freshness gate still rejects old data.
- 2026-09-16 — CRE report submission is single-attempt. A rerun must reread nonce and market state; receiver report-hash protection and policy nonce enforcement prevent an identical/replayed report from double execution. `submitReports` is false in checked-in targets. Fixture policy/L2 sources are part of the evidence identity and cannot be relabeled as deployed integration.
- 2026-09-16 — M3 uses one direct viem RPC event index for the initial immutable policy/receiver pair. It persists an atomic JSON projection with a block-number/hash cursor, verifies that cursor after restart, and performs a full deployment-block rebuild on identity or hash mismatch. Offchain engine/CRE decisions live in a separately typed journal and are never projected as mined events. Current expiry is read from the contract because no expiry event exists.
- 2026-09-16 — The M3 wallet boundary uses Privy embedded EVM wallets with visible user confirmation for every approval/create/cancel/revoke transaction. Monad Testnet `10143` is the only configured chain. No delegated signer or sponsor flag is enabled. The App ID and deployment addresses remain environment-only; missing configuration disables the action rather than switching to fixtures. Approval and revoke both target the immutable policy spender, while order policy remains separate from ERC20 allowance.
- 2026-09-16 — The create-order UI uses the M1 deployment scales directly: USDC amounts have 6 decimals, MON output has 18, and the contract price bound has 8. Approval is parsed and submitted independently from policy creation; it may be a finite amount different from the order budget. The UI warns when current allowance does not cover one maximum fill but does not misrepresent allowance as reserved spend or silently couple the two transactions.
- 2026-09-16 — Order pages recover event history from the atomic index file but treat a same-block contract read as lifecycle and policy authority. Index identity must match configured chain/policy/receiver, and immutable order fields must match the contract. Manual Kuru L2 is read at that policy block for display only; it is not a second estimator and still excludes AMM vault liquidity. Missing or inconsistent sources produce explicit blockers rather than stale lifecycle or fixture substitution.
- 2026-09-16 — Execution reports aggregate only indexed `ExecutionSettled` integers and compare them with current policy totals. Historical enrichment must match the successful transaction receipt, canonical block, and Kuru fee parameters at that fill block. Weighted BUY price rounds upward. A nonzero venue fee remains unavailable because net output is insufficient to reconstruct it exactly; Monad gas uses submitted gas limit multiplied by effective gas price and is never mixed with the USDC budget or venue fee.
- 2026-09-16 — Privy wallet attempt states persist in a versioned, validated browser-local journal so submitted/confirmed/failed/stale feedback survives an application restart. It is explicitly `LOCAL WALLET` provenance, is never projected into the onchain index, and does not create an explorer link when rejection occurred before a public transaction hash existed.
- 2026-09-16 — CRE/executor submission transitions are persisted separately from both the event projection and offchain decision journal. Order ID, nonce, proposal hash, source, timestamp, reason, status, and optional public transaction hash are mandatory provenance fields. Only the latest transition for a proposal is displayed; journal records cannot create fills or alter settlement accounting. Live production remains blocked until an official deployed CRE execution-history source can populate this boundary.
