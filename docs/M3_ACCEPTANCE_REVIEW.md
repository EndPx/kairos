...
| M3.4 Indexer | Canonical event projection, atomic checkpoint, restart, cursor-hash rebuild, and separate decision provenance | PASS — local | No public Kairos address exists for live ingestion |
...
| M3.8 Execution report | Actual settlement aggregation, weighted price, fill count/duration, separate venue fee/gas, fill-block receipt/market enrichment, explorer links, and local failed/stale wallet journal | PASS — local / PARTIAL live | No public indexed fill; nonzero venue fee cannot be reconstructed exactly from net output |
