...
| M3.5 Privy integration | Real pinned SDK boundary, embedded EVM wallet configuration, chain switch, balance/allowance reads, user-confirmed approve/create/cancel/revoke calls, honest missing-config gate, and an artifact-pinned lifecycle-only deployment path with two independent execution locks | PARTIAL | Browser runtime proves login, embedded-wallet presence, Monad Testnet `10143`, and native-balance reading. An initial `0 MON` observation correctly blocked deployment; a later fresh read showed `5 MON`, removing the gas-readiness blocker. No actual wallet action or public contract target exists. |
...
| PRIVY-01 | Real SDK transaction boundary, decoded calldata tests, authenticated embedded wallet, and correct network display | PARTIAL | Required actual approval/create/cancel/revoke wallet demonstration did not occur because no Kairos policy address is configured |
