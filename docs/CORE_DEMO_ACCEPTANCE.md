...
| Kuru fill | The production-shaped Kairos adapter path settled against deployed Kuru bytecode in six fixed-fork tests, including successful full/FOK and non-FOK partial fills | PROVEN on controlled local fork only | No exact build provenance, authorized public execution-capable deployment, or public Kuru settlement |
...
The immediate external dependency is the reproducible Kuru implementation provenance bundle defined in `docs/KURU_TECHNICAL_HANDOFF.md`. It is necessary to reconsider the public adapter gate but is not sufficient for the continuous demo: real CRE delivery identity/access and explicit deployment/transaction authorization remain separate requirements. Aurora is parked and is not used as a workaround for this public-fill path.
